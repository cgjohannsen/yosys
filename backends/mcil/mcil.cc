...
struct McilWorker
{
	...
	McilWorker(RTLIL::Module *module, bool verbose, std::ostream &f) :
...
//					f << stringf("    %s : unsigned word[%d]; -- neg(%s)\n", b_shl, GetSize(sig_b), log_signal(sig_b));
			...
			if (cell->type[0] == '$') {
				if (cell->type.in(ID($dffe), ID($sdff), ID($sdffe), ID($sdffce)) || cell->type.str().substr(0, 6) == "$_SDFF" || (cell->type.str().substr(0, 6) == "$_DFFE" && cell->type.str().size() == 10)) {
					log_error("Unsupported cell type %s for cell %s.%s -- please run `dffunmap` before `write_mcil`.\n",
				...
				if (cell->type.in(ID($adff), ID($adffe), ID($aldff), ID($aldffe), ID($dffsr), ID($dffsre)) || cell->type.str().substr(0, 5) == "$_DFF" || cell->type.str().substr(0, 7) == "$_ALDFF") {
					log_error("Unsupported cell type %s for cell %s.%s -- please run `async2sync; dffunmap` or `clk2fflogic` before `write_mcil`.\n",
							log_id(cell->type), log_id(module), log_id(cell));
				}
				if (cell->type.in(ID($sr), ID($dlatch), ID($adlatch), ID($dlatchsr)) || cell->type.str().substr(0, 8) == "$_DLATCH" || cell->type.str().substr(0, 5) == "$_SR_") {
					log_error("Unsupported cell type %s for cell %s.%s -- please run `clk2fflogic` before `write_mcil`.\n",
							log_id(cell->type), log_id(module), log_id(cell));
				}
...
struct McilBackend : public Backend {
	McilBackend() : Backend("mcil", "write design to MCIL file") { }
	void help() override
	{
		//   |---v---|---v---|---v---|---v---|---v---|---v---|---v---|---v---|---v---|---v---|
		log("\n");
		log("    write_mcil [options] [filename]\n");
		log("\n");
		log("Write an MCIL description of the current design.\n");
		log("\n");
		log("    -verbose\n");
	...
	void execute(std::ostream *&f, std::string filename, std::vector<std::string> args, RTLIL::Design *design) override
	{
		...
		log_header(design, "Executing MCIL backend.\n");
			...
			while (std::getline(template_f, line))
			{
				...
				if (line[indent] == '%')
				{
					...
					if (GetSize(stmt) == 2 && stmt[0] == "%module")
					{
						...
						*f << stringf("-- MCIL description generated by %s\n", yosys_version_str);

						log("Creating MCIL representation of module %s.\n", log_id(module));
						McilWorker worker(module, verbose, *f);
						worker.run();
